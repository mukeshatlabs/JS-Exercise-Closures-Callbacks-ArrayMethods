...
*/
function processLength(listArray, callback) {
  return callback(listArray.length);
}
...
// let sum = numberList.reduce((total, num) => {return total+num;},0);
...
// let list = [1,1,2,2,3];
// let newList = list.filter((a,b)=> list.indexOf(a) === b);
...
function firstNamesAllCaps(runners) {
  let runnerNames = runners.map((runner)=> runner.first_name.toUpperCase());
  return runnerNames;
}
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let runnersList = runners.filter((runner)=> (runner.shirt_size===tShirtSize));
  return runnersList;
}
...
function tallyUpDonations(runners) {
  let donations = runners.reduce((total,runner)=>(total+runner.donation),0);
  return donations;
}
